...
public class PlayerController2 : MonoBehaviour{
	...
	void Start(){
		...
		Global.countPlayer2 = 0; //setting count
	...
	void FixedUpdate(){
		...
		if(Input.GetKeyDown("left shift") && rb.transform.position.y <= 0.5f)
		{
			Vector3 jump = new Vector3(0.0f, 450, 0.0f);
			rb.AddForce(jump);
		}
	}
	...
	void OnTriggerEnter(Collider other){
		if (other.gameObject.CompareTag("Pick Up")){
			...
			Global.countPlayer2 = Global.countPlayer2 + 1;//increment the count for every collision, we want to use Unity's UI element to display the count
	...
	void SetCountText(){//updates count and potential win
		countText.text = "Player 2 Count: " + Global.countPlayer2.ToString();
	}
